...
class TFTBack(nn.Module):
    def __init__(self, config):
        ...
        self.quantile_proj = nn.Linear(config.hidden_size, config.temporal_target_size * len(config.quantiles))
...
class TemporalFusionTransformer(L.LightningModule):
    ...
    def criterion(self, pred, targets):
        # heat_loss = nn.functional.mse_loss(pred[:, :, 0], targets[:, :, 0])
        # cool_loss = nn.functional.mse_loss(pred[:, :, 0], targets[:, :, 1])
        heat_loss = self.quantile_loss(pred[:, :, :3], targets[:, :, 0].unsqueeze(2)).sum()
        cool_loss = self.quantile_loss(pred[:, :, 3:], targets[:, :, 1].unsqueeze(2)).sum()
    ...
    def training_step(self, batch, batch_idx):
        ...
        heat_loss, cool_loss = self.criterion(pred, targets)
