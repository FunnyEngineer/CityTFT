...
from utils.data_utils import InputTypes, DataTypes, FeatureSpec
import datetime
...
class ElectricityConfig():
    def __init__(self):
        ...
        self.num_static_vars = self.static_continuous_inp_size + \
            len(self.static_categorical_inp_lens)
        self.num_future_vars = self.temporal_known_continuous_inp_size + \
...
class TrafficConfig():
    def __init__(self):
        ...
        self.temporal_observed_continuous_inp_size = len(
            [x for x in self.features if x.feature_type == InputTypes.OBSERVED and x.feature_embed_type == DataTypes.CONTINUOUS])
        self.temporal_target_size = len(
            [x for x in self.features if x.feature_type == InputTypes.TARGET])
        self.static_continuous_inp_size = len(
            [x for x in self.features if x.feature_type == InputTypes.STATIC and x.feature_embed_type == DataTypes.CONTINUOUS])
...
class CitySimConfig():
    def __init__(self):
        ...
        self.features = [
            # climate (KNOWN)
            FeatureSpec('dm', InputTypes.KNOWN, DataTypes.CONTINUOUS),
            FeatureSpec('m', InputTypes.KNOWN, DataTypes.CONTINUOUS),
            FeatureSpec('h', InputTypes.KNOWN, DataTypes.CONTINUOUS),
            FeatureSpec('G_Dh', InputTypes.KNOWN, DataTypes.CONTINUOUS),
            FeatureSpec('G_Bn', InputTypes.KNOWN, DataTypes.CONTINUOUS),
            FeatureSpec('Ta', InputTypes.KNOWN, DataTypes.CONTINUOUS),
            FeatureSpec('Ts', InputTypes.KNOWN, DataTypes.CONTINUOUS),
            FeatureSpec('FF', InputTypes.KNOWN, DataTypes.CONTINUOUS),
            FeatureSpec('DD', InputTypes.KNOWN, DataTypes.CONTINUOUS),
            FeatureSpec('RH', InputTypes.KNOWN, DataTypes.CONTINUOUS),
            FeatureSpec('RR', InputTypes.KNOWN, DataTypes.CONTINUOUS),
            FeatureSpec('N', InputTypes.KNOWN, DataTypes.CONTINUOUS),
            # building (static)
            FeatureSpec('Roof_z', InputTypes.STATIC, DataTypes.CONTINUOUS),
            FeatureSpec('Wall_perimeter', InputTypes.STATIC, DataTypes.CONTINUOUS),
            FeatureSpec('Wall_aspect_ratio', InputTypes.STATIC, DataTypes.CONTINUOUS),
            FeatureSpec('Wall_GlazingRatio', InputTypes.STATIC, DataTypes.CONTINUOUS),
            FeatureSpec('Footprint_area', InputTypes.STATIC, DataTypes.CONTINUOUS),
            FeatureSpec('Zone_Tmax', InputTypes.STATIC, DataTypes.CONTINUOUS),
            FeatureSpec('Zone_Tmin', InputTypes.STATIC, DataTypes.CONTINUOUS),
            FeatureSpec('Wall_UValue', InputTypes.STATIC, DataTypes.CONTINUOUS),
            FeatureSpec('Roof_UValue', InputTypes.STATIC, DataTypes.CONTINUOUS),
            FeatureSpec('Floor_UValue', InputTypes.STATIC, DataTypes.CONTINUOUS),
            FeatureSpec('Wall_GlazingUValue', InputTypes.STATIC, DataTypes.CONTINUOUS),
            FeatureSpec('Wall_ShortWaveReflectance', InputTypes.STATIC, DataTypes.CONTINUOUS),
            FeatureSpec('Roof_ShortWaveReflectance', InputTypes.STATIC, DataTypes.CONTINUOUS),
            # Heating and cooling load (TARGET)
            FeatureSpec('heating', InputTypes.TARGET, DataTypes.CONTINUOUS), 
            FeatureSpec('cooling', InputTypes.TARGET, DataTypes.CONTINUOUS), 
        ]
        ...
        #### Derived variables ####
        self.temporal_known_continuous_inp_size = len(
            [x for x in self.features if x.feature_type == InputTypes.KNOWN 
             and x.feature_embed_type == DataTypes.CONTINUOUS])
        self.temporal_observed_continuous_inp_size = len(
            [x for x in self.features if x.feature_type == InputTypes.OBSERVED 
             and x.feature_embed_type == DataTypes.CONTINUOUS])
        self.temporal_target_size = len(
            [x for x in self.features if x.feature_type == InputTypes.TARGET])
        self.static_continuous_inp_size = len(
