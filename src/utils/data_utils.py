...
class TFTBinaryDataset(Dataset):
    def __init__(self, path, config):
        ...
        self.feature_type_col_map = [[i for i, f in enumerate(self.features) if (
            f.feature_type, f.feature_embed_type) == x] for x in FEAT_ORDER]
...
def get_dataset_splits(df, config):
    ...
    if hasattr(config, 'relative_split') and config.relative_split:
        ...
        for _, group in df.groupby(DEFAULT_ID_COL):
            ...
            _test = group.iloc[(len(_train) - config.encoder_length +
                                forecast_len):(len(_train) + 2*forecast_len)]
            train.append(_train)
            valid.append(_valid)
            test.append(_test)
...
def flatten_ids(df, config):
    ...
    if not (hasattr(config, 'combine_ids') and config.combine_ids):
        ...
        encoders = {c: sklearn.preprocessing.LabelEncoder().fit(
            df[c].values) for c in config.combine_ids}
        ...
        df[DEFAULT_ID_COL] = df.apply(lambda row: sum([encoders[c].transform(
            [row[c]])[0]*clens[i] for i, c in enumerate(encoders.keys())]), axis=1)
...
def normalize_reals(train, valid, test, config, id_col=DEFAULT_ID_COL):
    ...
    real_cols = list(set(v.name for v in config.features if v.feature_embed_type ==
                     DataTypes.CONTINUOUS).difference(set(tgt_cols)))
    real_scalers = {}
    tgt_scalers = {}
    ...
    def apply_scalers(df, name=None):
        if name is None:
            name = df.name
        mask = df.applymap(lambda x: True if x == config.missing_data_label else False) if hasattr(
            config, 'missing_data_label') else None
...
def encode_categoricals(train, valid, test, config):
    ...
    cat_cols = list(set(v.name for v in config.features if v.feature_embed_type ==
                    DataTypes.CATEGORICAL and v.feature_type != InputTypes.ID))
...
def preprocess(src_path, dst_path, config):
    ...
    col_names = [id_col] + [x.name for x in config.features if x.feature_embed_type !=
                            DataTypes.DATE and x.feature_type != InputTypes.ID]
    grouped_train = [x[1][col_names].values.astype(np.float32).view(
        dtype=np.int32) for x in train.groupby(id_col)]
    grouped_valid = [x[1][col_names].values.astype(np.float32).view(
        dtype=np.int32) for x in valid.groupby(id_col)]
    grouped_test = [x[1][col_names].values.astype(np.float32).view(
        dtype=np.int32) for x in test.groupby(id_col)]
...
def load_dataset(args, config, collate_fn=default_collate):
    ...
    if args.distributed_world_size > 1:
        data_sampler = DistributedSampler(train_split, args.distributed_world_size,
                                          args.distributed_rank, seed=args.seed + args.distributed_rank, drop_last=True)
    else:
    ...
    if args.distributed_world_size > 1:
        data_sampler = DistributedSampler(
            valid_split, args.distributed_world_size, args.distributed_rank, shuffle=False, drop_last=False)
    else:
        data_sampler = None
    ...
    if args.distributed_world_size > 1:
        data_sampler = DistributedSampler(
            test_split, args.distributed_world_size, args.distributed_rank, shuffle=False, drop_last=False)
    else:
        data_sampler = None
